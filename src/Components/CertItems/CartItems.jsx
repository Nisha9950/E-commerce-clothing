...
const CartItems = () => {
  ...
  const handleCheckout = () => {
    ...
    if (getTotalCartAmount() > 0) {
      ...
      if (isLoggedIn || storedUser) {
        toast.success("Proceeding to Shipping Method.", { autoClose: 2000 }); // Show success toast
        setToastShown(true); // Prevent multiple toasts
        setTimeout(() => navigate("/ShippingMethod"), 2000); // Navigate after toast
      } else {
        toast.warn("Please log in first!", { autoClose: 2000 }); // Show warning toast
        localStorage.setItem("redirectAfterLogin", "/paymentMethod");
        setTimeout(() => navigate("/login"), 2000);
      }
    } else {
      toast.error("Your cart is empty!", { autoClose: 2000 }); // Show error toast
    }
  };
